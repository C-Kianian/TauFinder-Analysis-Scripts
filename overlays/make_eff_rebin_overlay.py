...
parser.add_argument('--bibFile', type=str, required=True)
parser.add_argument('--noBIBFile', type=str, required=True)
parser.add_argument('--label1', type=str, default='BIB')
parser.add_argument('--label2', type=str, default='Non BIB')
parser.add_argument('--outdir', type=str, default='./rebin_pi_eff_overlay_plots')
parser.add_argument('--charge', type=str, required=True, choices=['plus', 'minus', 'both', 'none'],
...
eff_names = ['pt', 'eta', 'theta', 'phi', 'e']
variables = ['p_{T}', '|#eta|', '#theta', '#phi', 'E']
units = ['GeV/c', '', 'rad', 'rad', 'GeV']
...
for i in range(len(eff_names)):
    ...
    to_overlay = []
    for file in files:
        den = file.Get(f'mc_pion_{eff_names[i]}')
        makeD, makeS = False, False
        if to_include == 'both' or to_include == 'default':
            numDefault, makeD = file.Get(f'mc_matched_{eff_names[i]}'), True
        if to_include == 'both' or to_include == 'strict':
            numStrict, makeS = file.Get(f'mc_matched_{eff_names[i]}_strict'), True
        ...
        if name == 'pt':
            ...
            if makeD: to_overlay.append(make_eff(numDefault, den, f'{name}_eff', title, xtitle))
            if makeS: to_overlay.append(make_eff(numStrict, den, f'{name}_eff_strict', title, xtitle))
