...
for num, decay_mode in decay_modes.items():
    for i in range(3):
        ...
        #neutral overlay
        hDefault = root_file_default.Get(f'fake_{name}_{names[i]}_eff')
        hLoose = root_file_loose.Get(f'fake_{name}_{names[i]}_eff')
        hDynamic = root_file_dynamic.Get(f'fake_{name}_{names[i]}_eff')
        #hPion0 = root_file_default.Get(f'{name}_pi0_{names[i]}_eff')

        hDefault.SetTitle("Fake " + decay_mode + ' Rate Efficiencies vs ' + variables[i])
        #hDefault.SetTitle(f"Fake {num}-Prong Rate Efficiencies vs {variables[i]}")
        ...
        legend.AddEntry(hDynamic, '3/p_{T} Reco', 'lp')
        legend.AddEntry(hLoose, '6/p_{T} Reco', 'lp')
        legend.AddEntry(hDefault, 'Loose Reco', 'lp')
